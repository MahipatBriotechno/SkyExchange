            ...
            Object.entries(groupedGames).map(([cat, catGames]) => (
              <div key={cat} ref={el => sectionRefs.current[cat] = el} className="mb-8">
                <div className="flex items-center justify-between mb-3 border-l-4 border-[#ffb400] pl-3">
                  ...
                  <button 
                    onClick={() => scrollToSection(cat)}
                    className="text-[10px] font-bold text-[#ffb400] uppercase hover:underline"
                  >
                    View All
                  </button>
                </div>
...
const GameCard = ({ game, onLaunch, isSlider }) => {
  return (
    <div 
      onClick={onLaunch}
      className={`relative group bg-white rounded-md overflow-hidden shadow-sm border border-black/5 cursor-pointer active:scale-95 transition-all ${isSlider ? 'min-w-[120px] md:min-w-[160px] w-[120px] md:w-[160px]' : 'w-full'} aspect-[3/4]`}
    >
      <img 
